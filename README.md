...
外部客户端传入的工具不会注册或放行任何 OpenCode 同名内置工具。代理仅放行 OpenCode 的内部结构化输出工具，将模型生成的函数名和参数转换成 OpenAI `tool_calls`：
...
结构化工具决策需要完整生成后才能确定，因此带外部 `tools` 的 SSE 请求仍使用标准 SSE 格式，但 `tool_calls` 通常在流末尾一次发出；普通文本请求是真正的 token/文本增量流。
...
实现目标是 Chat Completions 核心兼容。`temperature`、`top_p`、`max_tokens` 等 OpenAI 采样参数目前不会覆盖 OpenCode 的 provider/model 配置；图片会在对话转录中作为 URL 提供给模型，而不是作为 OpenCode 原生附件上传。
